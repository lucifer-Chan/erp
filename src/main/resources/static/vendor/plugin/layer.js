...
window.layer = {
    ...
    use: function(module, fn, readyMethod){
        ...
        node[iscss ? 'href' : 'src'] = /^http:\/\//.test(module+'?120') ? module+'?120' : layer.path + module+'?120';
        node.id = id;
        if(!$('#'+ id)[0]){
    ...
    callback([
        ...
        '<div class="'+ doms[0] +' '+ (doms.anim[config.shift]||'') + (' layui-layer-'+ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin||'') +'" id="'+ doms[0] + times +'" type="'+ ready.type[config.type] +'" times="'+ times +'" showtime="'+ config.time +'" conType="'+ (conType ? 'object' : 'string') +'" style="z-index: '+ zIndex +'; width:'+ config.area[0] + ';height:' + config.area[1] + (config.fix ? '' : ';position:absolute;') +'">'
            ...
            + '<span class="layui-layer-setwin">'+ function(){
                var closebtn = ismax ? '<a class="layui-layer-min"><cite></cite></a><a class="layui-layer-ico layui-layer-max" ></a>' : '';
                config.closeBtn && (closebtn += '<a class="layui-layer-ico '+ doms[7] +' '+ doms[7] + (config.title ? config.closeBtn : (config.type == 4 ? '1' : '2')) +'" ><i class="fa fa-remove"></i></a>');
                return closebtn;
            }() + '</span>'
            + (config.btn ? function(){
                var button = '';
