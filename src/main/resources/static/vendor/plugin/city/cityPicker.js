...
IIInsomniaCityPicker.prototype = {
    init: function(initData){
        ...
        //初始化数据
        if(initData.provinceName && initData.cityName){
            that.settings.target.val(initData.provinceName + ',' + initData.cityName);
            that.settings.hideProvinceInput && that.settings.hideProvinceInput.val(initData.provinceCode);
            that.settings.hideCityInput && that.settings.hideCityInput.val(initData.cityCode);
        }
    ...
    cityEvent: function(){
        ...
        that.province_wrap.on('click', '.IIInsomnia-city', function(event) {
            ...
            if(that.settings.callback) 
                that.settings.callback(cid);
            else{
                that.settings.target.val(pname + ',' + cname);
                that.settings.hideProvinceInput && that.settings.hideProvinceInput.val(pid);
                that.settings.hideCityInput && that.settings.hideCityInput.val(cid);
            }
